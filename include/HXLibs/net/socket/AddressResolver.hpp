...
public:
    ...
    struct Address {
        ...
        ::socklen_t _addrlen = sizeof(::sockaddr_storage); // 初始化为 sockaddr_storage 的大小
     ...
     */
    struct AddressInfo {
        ::ADDRINFOA* _curr = nullptr; // 指向当前 addrinfo 结构体的指针

        /**
         * @brief 创建套接字、绑定它并监听连接
         * @return 服务器套接字
         */
        platform::SocketFdType createSocketAndBind() const {
            platform::SocketFdType serverFd = createSocket();
            AddressRef serveAddr = getAddress();
            int on = 1;
         ...
         * @return 服务器套接字
         */
        platform::SocketFdType createSocket() const {
            /** 注:
             * @brief `::socket` 创建一个套接字
             * 此函数用于创建一个新的套接字。
             * @param domain 协议族/域，如 AF_INET（IPv4）、AF_INET6（IPv6）、AF_UNIX（Unix 域）等
             ...
             * };
             */
            auto fd = ::socket(_curr->ai_family, _curr->ai_socktype, _curr->ai_protocol);
            if (fd == platform::kInvalidSocket) [[unlikely]] {
