...
#define _hx_REFL_STRUCT_MEMBER__(__NAME__) decltype(__NAME__) __NAME__;
...
#define _hx_REFL_GET_MEMBERS__(__ONE__, ...)                                   \
    t.__ONE__ __VA_OPT__(HX_FOR(_hx_REFL_GET_MEMBER__, __VA_ARGS__))

/// @brief 计算成员个数
#define _hx_ADD_1__(x) +1

/////////////////////////////////////////////////////////////

/// @brief 声明 __NAME__ 类型名称为 __NEW_NAME__ 的成员
#define _hx_REFL_STRUCT_MEMBER_AS__(__NAME__, __NEW_NAME__)                    \
...
/// @brief 把 ... 展开为 t.f0, t.f1, t.f2, ... 的形式
#define _hx_REFL_GET_MEMBERS_AS__(__ONE__, _, ...)                             \
    t.__ONE__ __VA_OPT__(FOR_EACH_PAIR(_hx_REFL_GET_MEMBER_AS__, __VA_ARGS__))

/// @brief 计算成员个数
#define _hx_ADD_2__(x, y) +1
...
#define HX_DIAGNOSTIC_IGNORE_CHANGES_MEANING                                   \
    _Pragma("clang diagnostic ignored \"-Wchanges-meaning\"")
#else
#define HX_DIAGNOSTIC_IGNORE_CHANGES_MEANING
#endif
...
#define HX_REFL(...)                                                           \
    inline static constexpr std::size_t membersCount() {                       \
        return static_cast<std::size_t>(HX_FOR(_hx_ADD_1__, __VA_ARGS__));     \
    }                                                                          \
    HX_DIAGNOSTIC_PUSH                                                         \
    HX_DIAGNOSTIC_IGNORE_CHANGES_MEANING                                       \
    inline constexpr static auto visit() {                                     \
        struct _hx_internal_ {                                                 \
            HX_FOR(_hx_REFL_STRUCT_MEMBER__, __VA_ARGS__)                      \
        };                                                                     \
        return ::HX::reflection::internal::                                    \
            ReflectionVisitor<_hx_internal_, membersCount()>::visit();         \
    }                                                                          \
    HX_DIAGNOSTIC_POP                                                          \
    template <typename T>                                                      \
    inline constexpr static auto visit(T& t) {                                 \
        return std::tie(_hx_REFL_GET_MEMBERS__(__VA_ARGS__));                  \
    }                                                                          \
    template <typename T>                                                      \
...
#define HX_REFL_AS(...)                                                        \
    inline static constexpr std::size_t membersCount() {                       \
        return static_cast<std::size_t>(                                       \
            FOR_EACH_PAIR(_hx_ADD_2__, __VA_ARGS__));                          \
    }                                                                          \
    HX_DIAGNOSTIC_PUSH                                                         \
    HX_DIAGNOSTIC_IGNORE_CHANGES_MEANING                                       \
    inline constexpr static auto visit() {                                     \
        struct _hx_internal_ {                                                 \
            FOR_EACH_PAIR(_hx_REFL_STRUCT_MEMBER_AS__, __VA_ARGS__)            \
        };                                                                     \
        return ::HX::reflection::internal::                                    \
            ReflectionVisitor<_hx_internal_, membersCount()>::visit();         \
    }                                                                          \
    ...
    template <typename T>                                                      \
    inline constexpr static auto visit(T& t) {                                 \
        return std::tie(_hx_REFL_GET_MEMBERS_AS__(__VA_ARGS__));               \
    }                                                                          \
    template <typename T>                                                      \
    inline constexpr static auto visit(T const& t) {                           \
        return std::tie(_hx_REFL_GET_MEMBERS_AS__(__VA_ARGS__));               \
...
#define HX_REFL_G(__CLASS_NAME__, ...)                                         \
    inline namespace _hx_reflection_metadata {                                 \
    ...
    visit(__CLASS_NAME__ const& t) {                                           \
        struct _hx_internal_ {                                                 \
            HX_FOR(_hx_REFL_STRUCT_G_MEMBER__, __VA_ARGS__)                    \
        };                                                                     \
        return ::HX::reflection::internal::ReflectionVisitor<                  \
            _hx_internal_,                                                     \
            ::HX::reflection::membersCountVal<_hx_internal_>>::visit();        \
    }                                                                          \
    HX_DIAGNOSTIC_POP                                                          \
    template <typename T>                                                      \
...
#define HX_REFL_G_AS(__CLASS_NAME__, ...)                                      \
    ...
    visit(__CLASS_NAME__ const& t) {                                           \
        struct _hx_internal_ {                                                 \
            FOR_EACH_PAIR(_hx_REFL_STRUCT_G_MEMBER_AS__, __VA_ARGS__)          \
        };                                                                     \
        return ::HX::reflection::internal::ReflectionVisitor<                  \
            _hx_internal_,                                                     \
            ::HX::reflection::membersCountVal<_hx_internal_>>::visit();        \
